...
from policy_explorer.supported_configuration.supported_regions_and_services import SUPPORTED_SERVICES
    ...
    try:
        ...
        if event['ServiceName'] in service_names:
            ...
            for policy in policies:
                policy.JobId = event['JobId']
            logger.info(f"Scanned policies for service {policies}")
            if policies:
        ...
        write_task_failure(
            ...
            None,
            event['ServiceName'],
            json.dumps({"error": f"function {err.name} is not available in ScanPolicyStrategy"})
        )
